...
const FROM_EMAIL = `"Smart Municipal Corporation" <${process.env.EMAIL_FROM || process.env.EMAIL_USER}>`;
    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto;">
          <div style="background: #0f4c75; padding: 24px; text-align: center; border-radius: 12px 12px 0 0;">
          ...
          <div style="background: #f9f9f9; padding: 30px; border-radius: 0 0 12px 12px; border: 1px solid #e0e0e0;">
            ...
            <p style="color: #555;">Your OTP for email verification is:</p>
            <div style="background: #0f4c75; color: white; font-size: 36px; font-weight: bold; text-align: center; padding: 20px; border-radius: 10px; letter-spacing: 10px; margin: 24px 0;">
              ${otp}
            </div>
            <p style="color: #888; font-size: 13px; text-align: center;">
              ⏰ Valid for <strong>10 minutes</strong>. Do not share this OTP.
            </p>
            <div style="text-align: center; margin-top: 20px;">
              <p style="color: #aaa; font-size: 12px;">Smart Municipal Corporation • Ahmedabad, Gujarat</p>
            </div>
          </div>
        </div>
      `,
...
exports.sendForgotPasswordEmail = async (email, otp, name) => {
  try {
    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto;">
          <div style="background: #e63946; padding: 24px; text-align: center; border-radius: 12px 12px 0 0;">
            <h2 style="color: white; margin: 0;">🔐 Password Reset Request</h2>
          </div>
          <div style="background: #f9f9f9; padding: 30px; border-radius: 0 0 12px 12px; border: 1px solid #e0e0e0;">
            <h3 style="color: #333;">Hello ${name}!</h3>
            <p style="color: #555;">Your OTP for password reset is:</p>
            <div style="background: #e63946; color: white; font-size: 36px; font-weight: bold; text-align: center; padding: 20px; border-radius: 10px; letter-spacing: 10px; margin: 24px 0;">
              ${otp}
            </div>
            <p style="color: #888; font-size: 13px; text-align: center;">
...
exports.sendStatusUpdateEmail = async (email, name, complaintData) => {
  try {
    ...
    const statusColors = {
      Pending: "#f57c00", Assigned: "#1b6ca8",
      Resolved: "#059669", Cancelled: "#e63946"
    };
    ...
    await transporter.sendMail({
      from: FROM_EMAIL,
      to: email,
      subject: `Complaint #${complaintId} — ${status} | SMC`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto;">
          <div style="background: #0f4c75; padding: 24px; text-align: center; border-radius: 12px 12px 0 0;">
          ...
          <div style="background: white; padding: 28px; border-radius: 0 0 12px 12px; border: 1px solid #e0e0e0;">
            ...
            <p style="color: #555;">Your complaint has been updated:</p>
            <div style="text-align: center; margin: 20px 0;">
              <span style="background: ${color}; color: white; padding: 10px 24px; border-radius: 24px; font-size: 16px; font-weight: bold; display: inline-block;">
                ${status}
              </span>
            </div>
            <table style="width: 100%; border-collapse: collapse;">
              <tr style="background: #f8fafc;">
                <td style="padding: 10px; font-size: 13px; color: #888; border-bottom: 1px solid #eee;">ID</td>
                <td style="padding: 10px; font-size: 13px; font-weight: 600; border-bottom: 1px solid #eee;">#${complaintId}</td>
              </tr>
              <tr>
                <td style="padding: 10px; font-size: 13px; color: #888; border-bottom: 1px solid #eee;">Type</td>
                <td style="padding: 10px; font-size: 13px; font-weight: 600; border-bottom: 1px solid #eee;">${type}</td>
              </tr>
              <tr style="background: #f8fafc;">
                <td style="padding: 10px; font-size: 13px; color: #888;">Area</td>
                <td style="padding: 10px; font-size: 13px; font-weight: 600;">${area}</td>
              </tr>
            </table>
            <div style="text-align: center; margin-top: 20px;">
...
exports.sendCertificateStatusEmail = async (email, name, certData) => {
  try {
    ...
    await transporter.sendMail({
      from: FROM_EMAIL,
      to: email,
      subject: `Certificate ${status} | SMC`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto;">
          <div style="background: #0f4c75; padding: 24px; text-align: center; border-radius: 12px 12px 0 0;">
            <h2 style="color: white; margin: 0;">📄 Certificate Update</h2>
          </div>
          <div style="background: white; padding: 28px; border-radius: 0 0 12px 12px; border: 1px solid #e0e0e0;">
            <p>Hello <strong>${name}</strong>,</p>
            <p>Your <strong>${type}</strong> certificate request #${certId} is: <strong>${status}</strong></p>
            <div style="text-align: center; margin-top: 20px;">
              <a href="${process.env.CLIENT_URL}/user/my-certificates"
                style="background: #0f4c75; color: white; padding: 12px 28px; border-radius: 8px; text-decoration: none; font-size: 14px; font-weight: 600;">
                View in Portal →
              </a>
            </div>
          </div>
...
exports.sendWaterStatusEmail = async (email, name, waterData) => {
  try {
    ...
    await transporter.sendMail({
      from: FROM_EMAIL,
      to: email,
      subject: `Water Request Update | SMC`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto;">
          <div style="background: #1b6ca8; padding: 24px; text-align: center; border-radius: 12px 12px 0 0;">
            <h2 style="color: white; margin: 0;">💧 Water Request Update</h2>
          </div>
          <div style="background: white; padding: 28px; border-radius: 0 0 12px 12px; border: 1px solid #e0e0e0;">
            <p>Hello <strong>${name}</strong>,</p>
            <p>Your ${type} request #${requestId} in <strong>${area}</strong> is: <strong>${status}</strong></p>
          </div>
        </div>
      `,
    });
...
exports.sendPointsEmail = async (email, name, pointsData) => {
  try {
    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto;">
          <div style="background: #f57c00; padding: 24px; text-align: center; border-radius: 12px 12px 0 0;">
            <h2 style="color: white; margin: 0;">⭐ Points Awarded!</h2>
          </div>
          <div style="background: white; padding: 28px; text-align: center; border-radius: 0 0 12px 12px; border: 1px solid #e0e0e0;">
            <p>Hello <strong>${name}</strong>,</p>
            <h2 style="color: #f57c00; font-size: 36px;">+${points} Points!</h2>
            <p style="color: #555;">Reason: ${reason}</p>
            <p style="color: #888;">Total: <strong>${totalPoints} pts</strong> = ₹${Math.floor(totalPoints / 100)} tax discount</p>
          </div>
        </div>
      `,
    });
